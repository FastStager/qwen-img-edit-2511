"""
RunPod serverless handler with TensorRT acceleration.

Loads TRT-compiled transformer if available, falls back to PyTorch.
All I/O optimizations (PyVIPS, thread pool) remain unchanged.

Env vars:
    MODELS_DIR  Base path for models (default: /workspace/models)
"""
...
TRT_ENGINE_PATH = f"{MODELS_DIR}/trt_engine/transformer.pt2"
TRT_CONFIG_PATH = f"{MODELS_DIR}/trt_engine/transformer_config.json"
...
def load_edit_model():
    ...
    if os.path.exists(TRT_ENGINE_PATH):
        try:
            ...
            print(f"--- Loading TRT engine from {TRT_ENGINE_PATH} ---")

            # Load pipeline (gets VAE, text_encoder, scheduler, tokenizer)
            ...
            # Grab config before replacing transformer
            transformer_config = pipe.transformer.config

            # Delete PyTorch transformer to free VRAM
            del pipe.transformer
            gc.collect()
            torch.cuda.empty_cache()

            # Load TRT engine
            pipe.transformer = torch_tensorrt.MutableTorchTensorRTModule.load(TRT_ENGINE_PATH)
            pipe.transformer.config = transformer_config

            USE_TRT = True
            print("--- TRT transformer loaded ---")

            # Print metadata if available
            meta_path = f"{MODELS_DIR}/trt_engine/metadata.json"
            if os.path.exists(meta_path):
                with open(meta_path) as f:
                    meta = json.load(f)
                print(f"--- TRT compiled on: {meta.get('gpu', '?')} | "
                      f"FP8: {meta.get('fp8', '?')} | "
                      f"Bench: {meta.get('benchmark_avg_s', '?')}s ---")
    ...
    if pipe is None:
        ...
        # QKV fusion (only for PyTorch path — TRT handles this internally)
        try:
            pipe.fuse_qkv_projections()
            print("--- QKV projections fused ---")
        except Exception as e:
            print(f"--- QKV fusion failed: {e} ---")

        # FP8 weight-only (only for PyTorch path — TRT has its own quantization)
        try:
    ...
    # ── Common optimizations (VAE — applies to both paths) ──
    try:
        pipe.vae.to(memory_format=torch.channels_last)
        print("--- VAE channels_last ---")
    except Exception:
        pass
    ...
    # FP8 VAE + text_encoder (both paths benefit)
    try:
        from torchao.quantization import quantize_, float8_weight_only
        cc = torch.cuda.get_device_capability()
        if cc[0] >= 8:
    ...
    # ── Warmup ──
    mode_str = "TRT" if USE_TRT else "PyTorch"
    print(f"--- Warmup ({mode_str}) ---")
...
def handler(job):
    ...
    # Decode + resize (PyVIPS, parallel for batch)
    try:
    ...
    cam_prompt = build_camera_prompt(
        job_input.get('azimuth'), job_input.get('elevation'), job_input.get('distance')
    )
    ...
    # Encode (PyVIPS, parallel for batch)
    def _encode(img):
    ...
    print(f"[TIMING:{mode}] decode={t_decode-t0:.3f}s prompt={t_preprocess-t_decode:.3f}s "
